...
LIBEA_MD_DECL(TASK_MUTATION_INSERTION_P, "ea.gls.task_mutation_insertion_p", double);
LIBEA_MD_DECL(TASK_MUTATION_DELETION_P, "ea.gls.task_mutation_deletion_p", double);
...
/*! Insertion/deletion mutation at a configurable rate.
 Probabilities are passed directly rather than read from EA metadata,
 allowing per-task scaling via a multiplier.
 */
struct configurable_indel {
    configurable_indel(double ins_p, double del_p) : _ins_p(ins_p), _del_p(del_p) {}

    template <typename EA>
    void operator()(typename EA::individual_type& ind, EA& ea) {
        typename EA::genome_type& repr = ind.repr();
        if (_ins_p > 0
...
struct task_mutagenesis : reaction_event<EA> {
    ...
    virtual void operator()(typename EA::individual_type& ind, // individual
        ...
        double ins_prob = get<TASK_MUTATION_INSERTION_P>(ea) * mult;
        double del_prob = get<TASK_MUTATION_DELETION_P>(ea) * mult;

        if (prob > 0) {
            configurable_per_site m(prob);
            mutate(ind,m,ea);
        }
        if (ins_prob > 0 || del_prob > 0) {
            configurable_indel im(ins_prob, del_prob);
            mutate(ind,im,ea);
        }
...
struct task_mutagenesis_control : reaction_event<EA> {
    ...
    virtual void operator()(typename EA::individual_type& ind, // individual
        ...
        double smallest_workload = get<WORKLOAD>(ind, 0.0);
        if (prob > 0 || ins_prob > 0 || del_prob > 0) {
