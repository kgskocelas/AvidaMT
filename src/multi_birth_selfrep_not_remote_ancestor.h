...
//h_alt_divide            repr[96] = ea.isa()["h_alt_divide"];
    ...
    // BUGGY (commented out — not used by any active executable as of 2026-04-06):
    // multibirth_selfrep_not_remote_ancestor_unfixed looks up ea.isa()["input"]
    // at repr[24] and repr[25], but all active configurations register "fixed_input",
    // not "input". The ISA [] operator will throw or return a garbage opcode,
    // producing a silently incorrect starting genome.
    //
    ...
    //
    //         // not
    //         repr[24] = ea.isa()["input"]; // BUG: should be "fixed_input"
    //         repr[25] = ea.isa()["input"]; // BUG: should be "fixed_input"
    //         repr[26] = ea.isa()["push"]; // push
    //         repr[27] = ea.isa()["nop_c"]; // nopc
    //         repr[28] = ea.isa()["pop"]; // pop
    //         repr[29] = ea.isa()["nand"]; // nand
    ...
    // Disabled: this unused experimental ancestor references
    // if_not_member_start_propagule and flag_1, which are not registered by the
    // active ISA setup in this repo's executables. Re-enabling it without also
    // registering those instructions will produce an invalid starting genome.
//    /*! Generates a self-replicating ancestor that performs not.
//     */
...
//    };
    ...
    // BUGGY (not used by any active executable as of 2026-04-06):
    // multibirth_selfrep_not_remote_unfixed_ancestor has the same bug as above:
    // ea.isa()["input"] at repr[24] and repr[25] will fail because all active
    // configurations register "fixed_input", not "input".
    //
