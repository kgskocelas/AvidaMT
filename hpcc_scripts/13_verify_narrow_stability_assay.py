#!/usr/bin/env python3
"""
verify_narrow_stability_assay.py

Verify output for the narrow-bracket stability assay.  Each seed's SLURM job
runs a variable number of costs (all whole integers strictly between the
last-non-reverting and first-reverting costs from the initial assay).
...
Expected directory structure:
    <base_dir>/
        ...
        narrow_trans_<seed>/
            cost_<N>/  ...
        narrow_final_<JOBID>_<seed>.log
        narrow_trans_<JOBID>_<seed>.log

Seeds are auto-detected by scanning for narrow_final_<N>/ or
narrow_trans_<N>/ directories.  Pass --seeds to override.

Because the number of costs varies per seed, completeness is judged by:
  - every cost_*/ directory that exists has all 3 data files non-empty
  - the log file for the seed contains "Done."

Usage:
    python3 verify_narrow_stability_assay.py [directory]
    python3 verify_narrow_stability_assay.py [directory] --condition final
    python3 verify_narrow_stability_assay.py [directory] --seeds 1007,1008,...
"""
...
def check_seed(seed: int, base_dir: Path, cond: Condition) -> tuple:
    ...
    status = "incomplete" if any_bad else "complete"
...
def _print_condition_results(cond, results, log_info, seeds):
    ...
    print(f"  {green(bold('Complete  '))}  {len(complete):>3} / {total}  "
          "(all cost dirs have all 3 files; variable count per seed)")
    ...
    if incomplete:
        ...
        for seed in incomplete:
            ...
            print(f"\n    {bold(yellow(f'Seed {seed}'))}  "
                  f"{n_ok}/{n_total} cost dirs OK  {log_tag}")
            ...
            if n_total == 0:
                print(f"      {red('No cost_*/ subdirectories found.')}")
...
def main():
    ...
    print(bold(f"Costs per seed : variable (whole integers in bracket)"))
