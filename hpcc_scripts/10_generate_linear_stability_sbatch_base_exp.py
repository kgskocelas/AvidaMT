#!/usr/bin/env python3
"""
generate_linear_stability_sbatch_base_exp.py

Variant of generate_linear_stability_sbatch.py configured for Heather's base
...
divide the gap into 8 equal chunks.
...
task is set to ceil(log2(4096 / min_cost)) * HOURS_PER_COST so the slowest
possible task still fits.
...
Output data directories (created at runtime):
    <work_dir>/linear_final_<seed>/cost_<N>/
    <work_dir>/linear_trans_<seed>/cost_<N>/

Usage:
    # Dry run — confirms brackets are read correctly from 008b, writes nothing
    python3 generate_linear_stability_sbatch_base_exp.py --dry-run

    # Generate sbatch files in current directory (defaults already set for base exp)
    python3 generate_linear_stability_sbatch_base_exp.py

    # Override where sbatch files are written
    python3 generate_linear_stability_sbatch_base_exp.py --output /path/to/sbatch/dir
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate linear-step stability assay sbatch scripts for base experiment (no indels).")
    ...
    seed_costs, skipped = collect_seed_costs(args.scan_dir)
    ...
    print(f"Total array tasks: {(n_final + n_trans) * N_COSTS}  (one per seed×cost pair)")
    ...
    for timepoint in ("final", "trans"):
        ...
        print(f"  Wrote: {fname}  ({n_tasks} tasks, wall time {WALL_TIME})")
